...
export class ApiKeyClient {
  ...
  /**
   * Получить баланс актива (например, USDT)
   */
  async getAsset(currency: string = 'USDT'): Promise<any> {
    const path = `/private/account/asset/${currency}`;
    const { timestamp, signature } = this.generateSignature('GET', path);

    try {
      console.log(`[API-KEY] Request: GET ${path}`);
      console.log(`[API-KEY] Headers: ApiKey=${this.apiKey.substring(0, 10)}..., Request-Time=${timestamp}, Signature=${signature.substring(0, 20)}...`);
      ...
      console.log(`[API-KEY] Response status:`, response.status);
      console.log(`[API-KEY] Response data:`, JSON.stringify(response.data).substring(0, 500));

      return response.data;
    } catch (error: any) {
      console.error(`[API-KEY] Error getting asset:`, error.response?.status);
      if (error.response?.data) {
        const responseText = typeof error.response.data === 'string' 
          ? error.response.data 
          : JSON.stringify(error.response.data);
