...
async function saveArbitrageSettings() {
    try {
        ...
        const settings = {
            ...
            symbol: finalSymbol // Сохраняем символ
        };
...
function initArbitrage() {
    ...
    api.getSettings().then(result => {
        if (result.success && result.data) {
            ...
            if (customSymbolEl && settings.symbol) {
